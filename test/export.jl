@testset "EDF Export" begin
    ...
    @testset "custom onda_to_edf transducer/prefilter" begin
        transducer_fn = sensor_type -> sensor_type == "eeg" ? "T1" : "T2"
        prefilter_fn = sensor_type -> sensor_type == "eeg" ? "0.9-20" : "0.1-0.3"
        exported_custom = onda_to_edf(samples_to_export,
                                      annotations;
        ...
        for samples in samples_to_export
            sensor_type = samples.info.sensor_type
            for (i, _) in enumerate(samples.info.channels)
                s = exported_custom.signals[custom_offset+i]
                @test s.header.transducer_type == (sensor_type == "eeg" ? "T1" : "T2")
                @test s.header.prefilter == (sensor_type == "eeg" ? "0.9-20" : "0.1-0.3")
            end
    ...
    @testset "custom transducer/prefilter" begin
        transducer_fn = sensor_type -> sensor_type == "eeg" ? "T1" : "T2"
        prefilter_fn = sensor_type -> sensor_type == "eeg" ? "0.9-20" : "0.1-0.3"
        custom_signals = OndaEDF.onda_samples_to_edf_signals(samples_to_export,
                                                             1.0,
    ...
    @testset "full service" begin
        ...
        @testset "$(samples_orig.info.sensor_type)" for (samples_orig,
            ...
            @test all(isapprox.(decode(samples_orig).data,
