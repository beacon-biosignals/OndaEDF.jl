...
specified in `plan_table` (e.g., as generated by [`plan_edf_to_onda_samples`](@ref)).
Returns:

- An `OrderedDict` containing the generated `Onda.Samples`, keyed by `sensor_label`.
- The plan as executed (which may be different from the input).
...
function edf_to_onda_samples(edf::EDF.File, plan_table; validate=true, dither_storage=missing)
    ...
    exec_rows = map(collect(grouped_plan_rows)) do (key, rows)
        try
            ...
            return (; key, samples, plan_rows=rows)
        catch e
            plan_rows = _errored_rows(rows, e)
            return (; key, samples=missing, plan_rows)
        end
    end
